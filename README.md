...
```
  ...
  docker run --rm --name accumulo --link zookeeper:zookeeper \
        --link hadoop:hadoop gchq/accumulo-gaffer:0.4.4g
  ...
  docker run -d --name wildfly --link zookeeper:zookeeper \
    ...
    -p 8080:8080 gchq/wildfly-gaffer:0.4.4g
...
```
  ...
  docker run -d --ip=10.10.10.10 --net my_network \
      ...
      --name acc01 gchq/accumulo-gaffer:0.4.4g
  ...
  docker run -d --ip=10.10.10.11 --net my_network \
      ...
      -e TRACER_HOSTS=10.10.10.10 \
      --link hadoop01:hadoop01 \
      --name acc02 gchq/accumulo-gaffer:0.4.4g
  ...
  docker run -d --ip=10.10.10.12 --net my_network \
      ...
      -e TRACER_HOSTS=10.10.10.10 \
      --link hadoop01:hadoop01 \
      --name acc03 gchq/accumulo-gaffer:0.4.4g
  ...
  docker run -d --name wildfly2 --ip=10.10.11.11 --net my_network \
      -e ZOOKEEPERS=10.10.5.10,10.10.5.11,10.10.5.12 \
      -p 8081:8080 gchq/wildfly-gaffer:0.4.4g
  docker run -d --name wildfly3 --ip=10.10.11.12 --net my_network \
      -e ZOOKEEPERS=10.10.5.10,10.10.5.11,10.10.5.12 \
      -p 8082:8080 gchq/wildfly-gaffer:0.4.4g
...
For configuration options on Accumulo, see <https://github.com/gchq/accumulo-docker/blob/master/README.md>.
...
Source is at <https://github.com/gchq/gaffer-docker>.
